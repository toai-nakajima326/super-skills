/**
 * hooks-gate.mts — TypeScript strict implementation of the AIOS hard-gate.
 *
 * Phase 1 of the hooks.js → .mts migration.
 * Spec: docs/specs/2026-04-20-ts-strict-migration-plan.md
 * Constitution: P2 (contract-first), P3 (fail-open), P6 (observe before act)
 *
 * What this module owns:
 *   - `get<T>()` returning `GetResult<T>`. Infra errors are a separate
 *     variant the caller MUST handle. The 2026-04-20 morning bug
 *     ({results:[]} swallowed all failures → gate blocked all AIOS
 *     writes) is now unrepresentable at compile time.
 *   - `sessionHasSkillUsage()` — throws `VcontextInfraError` on infra
 *     failure so `handlePreToolGate`'s catch can fail-open per §P3.
 *   - `handlePreToolGate()` — the actual gate decision, with
 *     discriminated-union narrowing on `tool_name`.
 *
 * What this module does NOT own (still in vcontext-hooks.js):
 *   - Recording events (`recordEvent`), subagent routing, session-recall,
 *     predictive-search, working-state, handoff, audit, skill-evolution.
 *   - Main dispatcher. hooks.js imports `handlePreToolGate` from here.
 *
 * Why self-contained helpers (duplicating some small pieces of hooks.js)?
 *   Minimal-risk Phase 1: zero edits to hooks.js except the one-line
 *   import swap. When phases 2-3 of the migration land, shared helpers
 *   move to `hooks-util.mts` and the duplication collapses.
 *
 * Runtime: Node 25+ strips types natively (no flag). Node 22-24 needs
 * `--experimental-strip-types`. Zero compile step. See
 * `~/Library/LaunchAgents/com.vcontext.hooks-setup.plist` for the
 * Claude Code entry point — it invokes `node scripts/vcontext-hooks.js
 * pre-tool`, which now imports this module.
 */
...
class VcontextInfraError extends Error {
  // Explicit field declaration instead of parameter property — Node's
  // strip-only type removal (--experimental-strip-types / Node 25 native)
  // cannot emit the implicit field-assignment for `public readonly x`
  // constructor args. Standard class shape works everywhere.
  readonly inner: GetError;
  constructor(inner: GetError) {
...
// ─────────────────────────────────────────────────────────────────────
// Exports consumed by scripts/vcontext-hooks.js
// ─────────────────────────────────────────────────────────────────────
//
// hooks.js imports `handlePreToolGate` via `await import(...)` (dynamic
// import, so the TypeScript strip cost is paid once per hook invocation
// only when pre-tool is hit). Type-only exports are elided at runtime.
