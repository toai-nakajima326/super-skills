...
`scripts/new-feature-watcher.cjs` が毎日 10:00 に実行される（LaunchAgent: `com.vcontext.new-feature-watcher`）。
...
```bash
# 実際に実行（vcontextに保存）
node ~/skills/scripts/new-feature-watcher.cjs

# ドライラン（保存しない）
node ~/skills/scripts/new-feature-watcher.cjs --dry-run
```
...
`scripts/conversation-skill-miner.cjs` が毎日 11:00 に実行される（LaunchAgent: `com.vcontext.conversation-skill-miner`）。
...
### 手動実行
```bash
node ~/skills/scripts/conversation-skill-miner.cjs          # 実行
node ~/skills/scripts/conversation-skill-miner.cjs --dry-run # 確認のみ
```
