...
## Workflow — Local AI Model Maintenance

### Step 0: Check Ollama availability
```bash
curl -s http://localhost:3150/ai/status
```
If `ollama_available: false`, skip this section entirely.

### Step 1: Update existing models
For each installed model, pull latest version:
```bash
...
```
Log which models were updated (version change).

### Step 2: Check for better models
Search the web for:
- "best local LLM for code summarization {year}"
- "ollama new models {year}"
- "best embedding model local M3 Mac {year}"

Evaluate against current models:
- **Is it better?** — benchmark scores, community consensus
- **Does it fit?** — must run on 36GB M3 (ideally <8GB for always-on, <20GB for on-demand)
- **Is it compatible?** — must be available via `ollama pull`

### Step 3: Install or skip
If a clearly better model is found:
```bash
ollama pull <new-model>
```
Then update the `MODEL_PREFS` in `vcontext-server.js` to use it, and reload:
```bash
./scripts/vcontext-reload.sh
```

### Step 4: Prune unused models
If a model was replaced and hasn't been used in 30+ days:
```bash
ollama rm <old-model>
```
Only if the replacement is confirmed working.

### Step 5: Log everything
Log to `~/skills/docs/evolution-log.md`:
- Models updated (version before → after)
- New models evaluated (adopted/skipped with reasoning)
- Models pruned
